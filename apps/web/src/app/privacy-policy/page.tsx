...
export default async function PrivacyPage() {
  const page = await apiGet<CmsPageDto>('/cms/pages/privacy-policy').catch(() => null);

  return (
    <>
      <PageHero heading={page?.title ?? 'Privacy Policy'} image={heroImages.privacy} />
      <div className="sf-container max-w-4xl py-12">
        {page?.body ? (
          ...
          <p className="rounded-2xl border border-[#E6EEF6] bg-white px-6 py-10 text-center text-[#6B7280]">
            The privacy policy has not been published in CMS yet.
          </p>
        )}
      </div>
    </>
